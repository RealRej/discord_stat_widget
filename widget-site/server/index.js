...
async function upsertCredentials(userId, { botToken, appId, discordUserId, platform }) {
  ...
  // undefined as a bind parameter. Blank botToken/platform means "leave the
  // existing one alone" — handled by the COALESCE below (falls back to
  // 'auto' only for a genuinely brand-new row with nothing to preserve).
  // This deliberately does NOT touch oauth_token_encrypted or
  // discord_linked — those are managed separately by the verify-token
  // endpoints, so editing other fields here doesn't wipe an existing link.
  ...
  // the bare column name for the existing row directly (not `excluded`,
  // which would be the incoming value here — the opposite of what's needed).
  const upsertSql =
    dialect === "pg"
      ? `INSERT INTO credentials (user_id, bot_token_encrypted, app_id, discord_user_id, platform)
         VALUES ($1, $2, $3, $4, COALESCE($5, 'auto'))
         ON CONFLICT (user_id) DO UPDATE SET
           bot_token_encrypted = COALESCE($2, credentials.bot_token_encrypted),
           app_id = $3, discord_user_id = $4,
           platform = COALESCE($5, credentials.platform), updated_at = now()`
      : `INSERT INTO credentials (user_id, bot_token_encrypted, app_id, discord_user_id, platform)
         VALUES ($1, $2, $3, $4, COALESCE($5, 'auto'))
         ON CONFLICT(user_id) DO UPDATE SET
           ...
           platform = COALESCE($5, platform), updated_at = datetime('now')`;

  await query(upsertSql, [userId, botTokenEncrypted, appId || null, discordUserId || null, platformValue]);
}
...
// Shared by both the self-service verify and the admin "verify on someone's
// behalf" action.
async function verifyAndSaveOauthToken(targetUserId, oauthToken) {
  const discordSelf = await fetchDiscordSelf(oauthToken);

  const { rows } = await query("SELECT discord_user_id FROM credentials WHERE user_id = $1", [targetUserId]);
  const savedDiscordUserId = rows[0] && rows[0].discord_user_id;
  if (savedDiscordUserId && savedDiscordUserId !== discordSelf.id) {
    const err = new Error(
      `That token belongs to Discord account ${discordSelf.id}, but the Discord User ID saved is ${savedDiscordUserId}. Fix the User ID field to match, or get a token for the right account.`
    );
    err.isMismatch = true;
    throw err;
  }
  ...
  await query(
    ...
    [encrypt(oauthToken), targetUserId]
  );
...
app.post("/api/discord/verify-token", requireLogin, async (req, res) => {
  try {
    ...
    const discordSelf = await verifyAndSaveOauthToken(req.session.userId, oauthToken);
    res.json({ ok: true, discordUsername: discordSelf.username });
  } catch (err) {
    console.error("[discord verify-token] failed:", err);
    res.status(err.isMismatch ? 400 : 502).json({ error: err.isMismatch ? err.message : `Discord rejected that token: ${err.message}` });
  }
});
...
app.post("/api/admin/users/:id/verify-token", requireAdmin, async (req, res) => {
  const targetId = Number(req.params.id);
  const { oauthToken } = req.body;
  if (!oauthToken) {
    return res.status(400).json({ error: "Paste an OAuth2 access token first." });
  }
  try {
    const discordSelf = await verifyAndSaveOauthToken(targetId, oauthToken);
    res.json({ ok: true, discordUsername: discordSelf.username });
  } catch (err) {
    console.error("[admin verify-token] failed:", err);
    res.status(err.isMismatch ? 400 : 502).json({ error: err.isMismatch ? err.message : `Discord rejected that token: ${err.message}` });
  }
});
