...
async function upsertCredentials(userId, { botToken, appId, discordUserId, platform }) {
  ...
  // undefined as a bind parameter. Blank botToken means "leave the
  // existing one alone" — handled by the COALESCE below.
...
app.post("/api/push/stream", requireLogin, requireSlot, async (req, res) => {
  ...
  try {
    const data = await lookupPlayer({
      ...
      skipSeasonBackfill: false, // push always does the full season backfill
      onProgress: (message, percent) => {
        if (activeSlot && activeSlot.userId === req.session.userId) activeSlot.lastSeen = Date.now();
        sendChunk(res, { type: "progress", message, percent });
      },
