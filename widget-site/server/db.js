...
async function initSchema() {
  ...
  // client_secret_encrypted is unused (left over from an earlier approach
  // that was abandoned in favor of a manually-pasted token) — harmless to
  // leave in place. oauth_token_encrypted holds that manually-pasted token,
  // and discord_linked tracks whether it's been successfully verified.
  await ensureColumn("credentials", "client_secret_encrypted", "TEXT");
  await ensureColumn("credentials", "oauth_token_encrypted", "TEXT");
  await ensureColumn("credentials", "discord_linked", "BOOLEAN NOT NULL DEFAULT FALSE");
  // Tracks which Riot account is currently pushed to this person's Discord
  // profile, updated on every successful push (whether they did it
  // themselves or an admin did it on their behalf) — shown in the admin panel.
  await ensureColumn("credentials", "riot_game_name", "TEXT");
  await ensureColumn("credentials", "riot_tag_line", "TEXT");
  ...
  // Holds matches that pagination has already discovered but that a
  // previous run didn't get to process within its time/count budget —
  // without this, a bounded run would "forget" everything past the first
  // batch and effectively never make progress past it.
  await ensureColumn("season_cache", "pending_match_ids", "TEXT NOT NULL DEFAULT '[]'");
