...
function defaultSeasonCache(puuid, queueId) {
  return { seasonStartIso: SEASON_START_ISO, puuid, queueId, processedMatchIds: [], pendingMatchIds: [], champions: {} };
}

async function loadSeasonCache(puuid, queueId) {
  const { rows } = await query(
    "SELECT season_start_iso, processed_match_ids, pending_match_ids, champions FROM season_cache WHERE puuid = $1 AND queue_id = $2",
    [puuid, queueId]
  );
  ...
  return {
    ...
    pendingMatchIds: JSON.parse(row.pending_match_ids || "[]"),
...
async function saveSeasonCache(cache) {
  await query(
    `INSERT INTO season_cache (puuid, queue_id, season_start_iso, processed_match_ids, pending_match_ids, champions)
     VALUES ($1, $2, $3, $4, $5, $6)
     ON CONFLICT (puuid, queue_id) DO UPDATE
       SET processed_match_ids = $4, pending_match_ids = $5, champions = $6, season_start_iso = $3`,
    [
      cache.puuid,
      cache.queueId,
      cache.seasonStartIso,
      JSON.stringify(cache.processedMatchIds),
      JSON.stringify(cache.pendingMatchIds),
      JSON.stringify(cache.champions),
    ]
  );
}
...
async function getSeasonMostPlayed(puuid, queueId, continentalRegion, platform, apiKey, onProgress = () => {}, shouldCancel = () => false) {
  ...
  // If a previous run discovered more matches than it had budget to process,
  // resume that exact leftover queue first — do NOT re-run pagination here,
  // since pagination would immediately hit these same (still-unprocessed,
  // but already-discovered) matches and short-circuit before ever reaching
  // the older ones still waiting behind them.
  let queue = cache.pendingMatchIds.slice();

  if (queue.length === 0) {
    // Fully caught up as of last run — safe to do a fresh discovery pass to
    // pick up anything genuinely new since then.
    ...
    onProgress(`Resuming ranked history sync — ${queue.length} games queued from last time...`, null);
  }
  ...
  for (let i = 0; i < queue.length; i++) {
    ...
    if (processedThisRun >= BACKFILL_MAX_MATCHES_PER_RUN || Date.now() - runStart > BACKFILL_MAX_DURATION_MS) {
      stillPending = queue.slice(i); // whatever never got processed becomes next run's queue — nothing gets lost
      partial = true;
      break;
    }
    ...
    try {
      ...
      const me = match.info.participants.find((p) => p.puuid === puuid);
      if (!me) {
        stillPending.push(matchId); // couldn't read it this time, retry next run rather than lose it
      } else {
      ...
      stillPending.push(matchId); // network/API hiccup — retry next run rather than lose it
    }
    processedThisRun++;

    const remaining = queue.length - processedThisRun;
    const etaSeconds = remaining * (SEASON_FETCH_DELAY_MS / 1000);
    onProgress(
      remaining > 0
        ? `Analyzing ranked history — match ${processedThisRun} of ${queue.length} (~${formatDuration(etaSeconds)} left)`
        : `Analyzing ranked history — match ${processedThisRun} of ${queue.length}`,
      processedThisRun / Math.max(queue.length, 1)
    );
    await sleep(SEASON_FETCH_DELAY_MS);
  }

  cache.pendingMatchIds = stillPending;
