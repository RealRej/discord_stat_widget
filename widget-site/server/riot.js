...
async function getPeakRankFromOpgg(gameName, tagLine, serverLabel) {
  ...
  try {
    ...
    const [tierRaw, divisionRaw] = match[1].trim().split(/\s+/);
    const lp = match[2];

    // Match the same format Riot's own API uses for current rank — e.g.
    // "EMERALD IV - 76 LP" — instead of op.gg's own mixed-case, Arabic-numeral
    // style ("Emerald 3"). Apex tiers (Master/GM/Challenger) have no division
    // on op.gg either, so default to "I" the same way Riot's API does.
