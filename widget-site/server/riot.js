...
async function fetchAllMatches(matchIds, continentalRegion, apiKey, onMatch = () => {}) {
  const matches = [];
...
async function getUnrankedFallbackMatches(puuid, countPerQueue, continentalRegion, platform, apiKey, onMatch = () => {}) {
  const allIds = [];
  ...
  const matches = await fetchAllMatches(allIds, continentalRegion, apiKey, onMatch);
  matches.sort((a, b) => {
...
async function getSeasonMostPlayed(puuid, queueId, continentalRegion, platform, apiKey, onProgress = () => {}) {
  ...
  for (let i = 0; i < newMatchIds.length; i++) {
    ...
    onProgress(`Analyzing ranked history — match ${i + 1} of ${newMatchIds.length}`, (i + 1) / Math.max(newMatchIds.length, 1));
    await sleep(SEASON_FETCH_DELAY_MS);
  }
...
async function lookupPlayer({ apiKey, gameName, tagLine, platformPref = "auto", skipSeasonBackfill = false, onProgress = () => {} }) {
  // Percent ranges are split differently depending on whether the season
  // backfill will run: that step dominates total time on active accounts,
  // so it gets most of the bar when it's happening, and the other stages
  // shrink accordingly so the bar doesn't stall at a low number for ages.
  const P = skipSeasonBackfill
    ? { account: 5, platform: 15, core: 30, matches: 65, analyze: 75, peak: 90, splash: 98 }
    : { account: 3, platform: 8, core: 15, matches: 25, analyze: 30, backfillStart: 30, backfillEnd: 88, peak: 93, splash: 98 };
  ...
  const matches = activeQueue
    ...
    : await getUnrankedFallbackMatches(account.puuid, 20, continentalRegion, platform, apiKey, onMatchProgress);
  ...
  const mostPlayed =
    activeQueue && !skipSeasonBackfill
      ? await getSeasonMostPlayed(account.puuid, activeQueue.queueId, continentalRegion, platform, apiKey, (message, fraction) => {
