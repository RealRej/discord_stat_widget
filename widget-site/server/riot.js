...
async function getSeasonMostPlayed(puuid, queueId, continentalRegion, platform, apiKey, onProgress = () => {}) {
  ...
  return summarizeChampions(cache.champions);
}
...
async function lookupPlayer({ apiKey, gameName, tagLine, platformPref = "auto", onProgress = () => {} }) {
  ...
  if (activeQueue) {
    try {
      mostPlayed = await getSeasonMostPlayed(account.puuid, activeQueue.queueId, continentalRegion, platform, apiKey, (message, fraction) => {
